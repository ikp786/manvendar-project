...
Route::any('amazon-callback-url', 'CallBackUrlController@AmazonResponseCallBack');
 
$this->any('reset-again', 'Auth\LoginController@force_redirect')->name('reset-again');
$this->any('verify-again', 'Auth\LoginController@verify_again_redirect')->name('verify-again');

Route::post('generateTransactionpinFrontEnd', 'Auth\LoginController@generateTransactionpinFrontEnd'); 

 
 // Route::get('force_redirect', 'Auth\LoginController@force_redirect');
 
   // Route::post('password/reset-again', 'Auth\LoginController@force_redirect')->name('reset-again');
 
//	Route::any('a2z-paytm-callback-url/{response}','CallBackUrlController@PayTMResponseCallBack');  // Paytm call back api url
  	Route::any('a2z-paytm-callback-url','CallBackUrlController@PayTMResponseCallBack');  // Paytm call back api url	
	 	
	//Route::get('a2z-paytm-callback-url/{response}','MyWalletController@PayTMResponseCallBack'); // for paytm call back url
...
Route::group(['middleware'=>['logout-after-user-inactive','auth']],function() 
{
	...
	Route::get('/admin/php_info', 'MemberController@php_info');
	...
	Route::get('aeps-ap', 'AepsController@apaepsLogin')->name('aeps-ap');
    ...
    Route::post('onboarding_apicall','ApiForwarding\AepsRegistrationController@aepsMemberRegistration')->name('onboarding_apicall');
	...
	Route::post('send-apaeps-request', 'AepsController@sendAPRequest')->name('send-apaeps-request');
	...
	/******** Adhaar pay ******/
	    ...
	    Route::get('adhaar-pay-scheme', 'WalletSchemeController@getAllAdhaarPayScheme');
	    Route::post('/adhaar-pay-commission-update', 'CommissionController@adhaarPayViewScheme');// adhaar pay commision
    	// Route::post('add-new-aeps-scheme', 'WalletSchemeController@createAepsScheme')->name('add-new-aeps-scheme');
        Route::post('create-new-adhaar-pay-row','WalletSchemeController@createAdhaayPayOneRow')->name('create-new-adhaar-pay-row');
    	Route::post('updateAdhaayPayScheme','WalletSchemeController@updateAdhaarPayScheme');
	...
	Route::get('a2z-plus-scheme', 'WalletSchemeController@getAllA2zPlusScheme');
	Route::post('/a2z-plus-wallet-commission-update', 'CommissionController@a2zPlusViewScheme');
	
	/*
    	Route::post('/newChangePassword', function() {
            $user=session('user');
            dd($user);
            return view('profileNewUser', $user);
        })->name('newusr');
        
        Route::post('my-wallet-paytm-transaction','MyWalletController@paytmTransaction');
    */
   
    
	Route::get('my-wallet', 'MyWalletController@index')->name('my-wallet');
	Route::get('my-wallet-mobile-verification','MyWalletController@mobileVerificaton');
	Route::get('my-wallet-remitter-register','MyWalletController@remitterRegister');
	Route::put('my-wallet-mobile-verification-with-otp/{mobile}','MyWalletController@mobileVerifiedWithOTP');
	Route::get('my-wallet-get-bene-list','MyWalletController@getBeniList');
	Route::post('my-wallet-bene-add','MyWalletController@beneAdd');
	Route::get('my-wallet-get-agent-charge-amt', 'MyWalletController@getAgentChargeAmount');
	Route::get('my-wallet-check-is-bank-down','MyWalletController@isBankDownOrNot');
	Route::post('my-wallet-paytm-transaction','MyWalletController@paytmTransaction');
	Route::put('my-wallet-bene-delete-request/{beneId}','MyWalletController@deleteBeneficiaryRequest');
	
	Route::put('my-wallet-confirm-bene-delete','MyWalletController@deleteBeneficiaryThroughOtp');
	
	// Route::post('bank-transfer','MyWalletController@paytmTransaction');
	/* Premium Wallet  */
	...
	Route::post('check-complain-status','ComplainController@checkComplainStatus');
	...
	Route::get('admin/member_permission/{Type?}/{BackTo?}', 'MemberController@member_permission');

Route::post('admin/member_permission/insert_data', 'MemberController@save_permission_data'); 
	...
	Route::get('admin/all-members/{type?}', 'MemberController@index');
	
	Route::get('admin/all-remitters', 'MemberController@remitter_list');
	// Route::any('admin/remitters-send', 'MemberController@send_message_to_remitter');
	Route::post('admin/remitters-send', 'MemberController@send_message_to_remitter');
	//	Route::post('recharge-scheme', 'SchemeController@createRechargeScheme');
...
Route::group(['prefix' => 'mobileapp/api'], function (){
    //login
     Route::post('money-transfer-a2z-plus-wallet', 'Mobile\MyWalletController@paytmTransaction'); 
     Route::group(['prefix' => 'a2zplus'], function () {

        Route::get('mobile-verification', 'Mobile\MyWalletController@mobileVerificaton');
        Route::post('remitter-registration', 'Mobile\MyWalletController@remitterRegister');
        Route::post('mobile-verification-otp', 'Mobile\MyWalletController@mobileVerifiedWithOTP');
        Route::get('get-beniList', 'Mobile\MyWalletController@getBeniList');
         Route::post('bene-add', 'Mobile\MyWalletController@beneAdd');
         
         Route::post('delete-beneficiary', 'Mobile\MyWalletController@deleteBeneficiaryRequest');
         Route::post('delete-beneficiary-otp', 'Mobile\MyWalletController@deleteBeneficiaryThroughOtp');
     });
