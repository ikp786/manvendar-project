...
<?php echo $__env->make('agent.report.report-type', \Illuminate\Support\Arr::except(get_defined_vars(), array('__data', '__path')))->render(); ?>
	...
	<div class="col-md-12">
        <form method="get" action="<?php echo e(Request::url()); ?>" class="form-inline" role="form">    
			...
			<input name="fromdate" class="form-control customDatepicker" type="text" placeholder="From date" value="<?php echo e((app('request')->input('fromdate')) ? app('request')->input('fromdate') : date('d-m-Y')); ?>">
		
			<input name="todate" class="form-control customDatepicker" type="text" placeholder="To date" value="<?php echo e((app('request')->input('todate')) ? app('request')->input('todate') : date('d-m-Y')); ?>">
			<?php if(in_array(Auth::user()->role_id,array(5))): ?>
			<?php echo e(Form::select('searchOf', ['1' => 'Success', '2' => 'Failed','3'=>"Pending",'4'=>"Refunded",'24'=>"Successfull",'21'=>"Refund Success",'18'=>"In-Process",'6'=>'DT'], (app('request')->input('searchOf')), ['class'=>'form-control','placeholder'=>"--Select Status--"])); ?>

			
			<?php echo e(Form::select('product', ['1' => 'Recharge/Bill Payment', '2' => 'Verification','4' => 'DMT 1','16' => 'DMT 2','5'=>"A2Z wallet",'10'=>'AEPS'], (app('request')->input('product')), ['class'=>'form-control','placeholder'=>"--Select Product--"])); ?> 
			<?php else: ?>
		...
		<table  class="table table-bordered table-striped" id="example2">
			...
			<tbody>
				...
				<tr class="<?php echo e($recharge_reports->status->status); ?>-text">
					...
					<td><?php echo e($recharge_reports->number); ?><p><span style="font-weight:bold;">					<?php echo e(@$recharge_reports->beneficiary->name); ?></p>
						<?php if(@$recharge_reports->api_id == 2): ?>
							<?php echo e(@$recharge_reports->biller_name); ?>
				   ...
				   <?php if(in_array($recharge_reports->api_id,array(2,10))): ?>
					...
					<td><?php echo e($recharge_reports->txnid); ?><br><?php echo e($recharge_reports->remark); ?></td>
					...
					<td><?php if($recharge_reports->status_id !=4): ?>
						...
						<?php endif; ?>						<?php if(in_array($recharge_reports->status_id,array(1,3,9,18)) && $recharge_reports->api_id !=10 ): ?>
					...
					<?php if(in_array($recharge_reports->status_id,array(1,3,9,18,24))): ?>
