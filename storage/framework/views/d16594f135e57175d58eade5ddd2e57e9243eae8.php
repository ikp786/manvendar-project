...
<!-- for popup -->
	...
	$(document).ready(function()
    {
        $("#txnStatus").change(function() 
		{
            ...
            if($(this).val() == "2" || (status_id == 1 && description=="CASH_WITHDRAWAL")) {
                $("#txn_otp").show();
                $("#btn-save").hide();
				$("#VerifyOtp").hide();
            }
    ...
    <div class="panel-body">
        <form method="get" action="<?php echo e(Request::url()); ?>" class="form-inline" role="form" >
			...
			<?php if(Auth::user()->role_id == 1): ?> 
				<?php echo e(Form::select('searchOf', ['1' => 'Success', '2' => 'Failed','3'=>"Pending",'4'=>"Refunded",'21' => 'Manual Success', '28' => 'Manual Failed','18'=>'InProcess','6'=>'Debit','7'=>'Credit','100'=>'Manual Success','101'=>'Manula Failed'], app('request')->input('searchOf'), ['class'=>'form-control','placeholder'=>'--Select Status--'])); ?>


				<?php echo e(Form::select('user', @$users, app('request')->input('user'), ['class'=>'form-control', 'placeholder'=>"----Select User----"])); ?>

				<?php echo e(Form::select('product', ['4' => 'DMT1', '5' => 'A2Z wallet', '16' => 'DMT 2','10' => 'AEPS','1' => 'BBPS','13'=>'Suvidhaa','2'=>"INS Verify",'17'=>"A2Z Verify"], app('request')->input('product'), ['class'=>'form-control','placeholder'=>'--Select Product--'])); ?>                         
                ...
                <tbody>
					...
					<?php $s = $value->created_at;
						...
						<tr class="<?php echo e($value->status->status); ?>-text">
							...
							<td><?php echo e($value->number); ?><br>
                                <span style="font-style: bold;"><?php echo e(@$value->beneficiary->name); ?></span></td>
                            <td>
					       ...
					       <?php else: ?>
                            ...
                            <td><?php echo e($value->status->status); ?>

							<p><?php if($value->recharge_type==0 && $value->txnid !="DT" && $value->api_id !=2): ?> 
							...
							<td>
								<?php if(in_array($value->status_id,array(1,3,9,18))): ?>
                                ...
                                Check</a> 
                                <img src="<?php echo e(url('loader/loader.gif')); ?>" id="checkImg_<?php echo e($value->id); ?>" class="loaderImg" style="display: none;"><?php endif; ?>
								<?php endif; ?>
                            </td> 
							<td>
							<?php if(Auth::user()->role_id == 1): ?>
