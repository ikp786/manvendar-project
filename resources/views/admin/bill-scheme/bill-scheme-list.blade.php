    ...
    function CommissionPage(id){
        ...
        <div class="col-sm-12">
            <div class="col-lg-6 col-md-6">
                <h4 class="page-title" style="color: black;">{{  'BILL SCHEME DETAIL' }}</h4>
   ...
   <div class="panel-body">
    <div class="table table-responsive">
        ...
        <table border='1' class="table table-striped table-bordered table-hover" id="dataTables-example">
					...
					<tbody>
					   @foreach($impsSchemeLists as $key => $value)
						<tr class="odd gradeX">
							<td>{{ $value->id }}</td>
							<td>{{ $value->name }}</td>
                            ...
                            <td>{{ Form::open(array('url' => 'bill-pay-commission-update')) }}
                                {{ Form::hidden('schemeName', $value->name) }}
                                {{ Form::hidden('id', $value->id) }}
                                {{ Form::hidden('scheme_for', $value->scheme_for) }}
                                {{ Form::submit('View Bill SCHEME', array('class' => 'btn btn-primary')) }}
                                {{ Form::close() }}
                            </td>
            ...
            <div class="modal-body">
               ...
               {!! Form::open(array('url' =>'#','id'=>'newMoneyScheme')) !!}
					...
					<div class="form-group">
                        ...
                        <div class="col-sm-9">
                            {{ Form::select('scheme_for', ['5' => 'BILL Payment Scheme'], null, ['class'=>'form-control']) }}
                        </div>
                    </div>
