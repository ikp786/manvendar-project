  ...
  <script src="//cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.6.3/js/bootstrap-select.min.js"></script>
    ...
    <div class="panel-body">
        <form method="get" action="{{ Request::url() }}" class="form-inline" role="form" >
            ...
            <input name="fromdate" style="width:140px" class="form-control customDatepicker" type="text" value="{{ (app('request')->input('fromdate')) ? app('request')->input('fromdate') : date('d-m-Y')}}" autocomplete="off"> 
            
            <input name="todate" style="width:140px" class="form-control customDatepicker" type="text" value="{{ (app('request')->input('todate')) ? app('request')->input('todate') : date('d-m-Y')}}" autocomplete="off">
            {{Form::select('mode', ['APP' => 'APP', 'WEB' => 'WEB'], (app('request')->input('mode')), ['class'=>'form-control select2-single','placeholder'=>"--Select Mode--"])}} 
			@if(Auth::user()->role_id == 1) 
				{{ Form::select('searchOf', ['34' => 'Accepted','1' => 'Success', '2' => 'Failed','3'=>"Pending",'4'=>"Refunded",'21' => 'Refund Success', '28' => 'Manual Failed','18'=>'InProcess','6'=>'Debit','7'=>'Credit','100'=>'Manual Success','101'=>'Manula Failed'], app('request')->input('searchOf'), ['class'=>'form-control','placeholder'=>'--Select Status--']) }}

                <div style="width:200px !important;float: left;  padding-right: 4px;">                                                        
				{{ Form::select('user', @$users, app('request')->input('user'), ['data-live-search'=>'true','data-show-subtext'=>'true', 'id'=>'user_list','class'=>'form-control selectpicker', 'placeholder'=>"----Select User----"]) }}
				</div>
					...
					<?php $s = $value->created_at;
						...
						<tr class="{{@$value->status->status}}-text">
					       ...
					       @else
							...
							<td> 
								@if(in_array($value->status_id,array(1,2,3,9,18,34)))
                                    @if($value->api_id==10)
                                    ...
                                    @else
                						...
                						@else
                						    @if($value->api_id=='27')
                						        <a href="javascript::void(0)" disabled class="btn btn-outline-info btn-sm" id="checkBtn_{{$value->id}}">
                                                Check</a> 
                                            @else
                                                ...
                                                Check</a>
                                            @endif
                                            <img src="{{url('loader/loader.gif')}}" id="checkImg_{{$value->id}}" class="loaderImg" style="display: none;">
                						@endif 
                                    @endif
								@endif
                            </td> 
							<td>
							@if(Auth::user()->role_id == 1)
								@if(in_array($value->status_id,array(1,3,9,18,34)))
								<a data-toggle="modal" href="#example" class="table-action-btn" onclick="updateRecord({{$value->id}})">Action</a>
								@endif
								@if(in_array($value->status_id,array(1,2,3,9,20,21,24,32,34)))
