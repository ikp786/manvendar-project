            ...
            <div class="col-md-6">
                <form method="get" action="{{ Request::url()}}" class="form-inline"
                    ...
                    <div class="form-group">
                        {{ Form::select('SEARCH_TYPE', ['NAME'=>'NAME','ID' => 'Record Id','MOB_NO'=>'Mobile No'], null, ['class'=>'form-control', 'style'=>"height: 10%;"]) }}
                    </div>
                     <div class="form-group">
                     ...
                     <div class="">
						<table id="mytable"  class="table table-bordered " >
                            ...
                            @foreach($users as $user)
                                <tr>
                                   <!-- <td>{{ $user->id }}</td>-->
                                    <td>{{ ($user->role_id == 4) ? "D " : "R " }}{{ $user->id }}</td>
                                    <td>{{ $user->name }}</td>
									...
									<td><textarea style="color:black;" class="expand" onclick="expand_textarea()" id="remark_{{ $user->id }}" rows="2" cols="7" name="remark"></textarea></td>
                                    @if(Auth::user()->company->company_id!=13)
                                    <!--<td><a id="trbutton_{{ $user->id }}" onclick="transfernow({{ $user->id }})" class="btn btn-success">Return Now</a> </td>-->
									<td><a id="trbutton_{{ $user->id }}" onclick="openVerificationPin({{ $user->id }},'DT')" class="btn btn-success">Return Now</a> </td>
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                     <button onclick="pinVerification()" id="pinVerificationBtn" type="button" class="btn btn-info waves-effect waves-light" value="add">Verify
                    </button>
