...
@section('content')
    <script>
        ...
        function updateRecord(id) {
            ...
            $.ajax({
                type: "post",
                url: "{{url('scheme-manage/cashback')}}",
                data: dataString,
                success: function (data) {
                    alert(data);
                    return false;
                    ...
                    @foreach($cashback as $value)
                        ...
                        <tr>
                            ...
                            <!--  <select class="form-control" name="cashback_category" id="cashback_category_{{ $value->id }}"
                                ...
                                @if($value->categories=='BILL')
                                 <input type="text" style="background:orange;" disabled class="form-control" name="cashback_category1" value="{{$value->categories}}">
                                @else
