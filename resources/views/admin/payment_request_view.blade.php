...
@section('content')
    <script>
        ...
        function updateRecord(id) {
            ...
            $.ajax({
                type: "post",
                url: "{{url('payment-request-view/view')}}",
                data: dataString,
				beforeSend:function()
				{
					 $('#adminRemark').val('');
...
function getRecords(user_id)
		{
			...
			$.ajax({
                type: "get",
                url: "{{url('payment-request-view/getRecords')}}",
                data: dataString,
		...
		function getLastTenApprovedAmount(balance)
        {
            ...
            $.ajax({
                type: "get",
                url: "{{url('payment-request-view/get-last-ten-approved-amount')}}",
                data: dataString,
                success: function (result) {
                    if(result.status == 1){
		...
		function addNewRemark()
        {
                ...
                if(remark)
                {
                    ...
                    $.ajax({
                    type: "POST",
                    url: "{{url('payment-request-view/addNewRemark')}}",
                    data: dataString,
                    success: function (result) {
                        if(result.status == 1){
        ...
        function editRemark()
        {
            ...
            var remark = prompt("Enter New Remark to update");
                if(remark)
                {
                     ...
                     $.ajax({
                            type: "put",
                            url: "{{url('payment-request-view/edit-remark')}}"+'/'+remark_id,
                            data: dataString,
                            success: function (data) 
                            {
                               alert(data.message);
        ...
        function deleteRemark()
        {
            ...
            if(confirm("Are you sure to delete selected remark"))
            {
                     ...
                     $.ajax({
                            type: "post",
                            url: "{{url('payment-request-view/delete-remark')}}",
                            data: dataString,
                            success: function (data) {
                                alert(data.message);
                               //$('#remark_id option[value=2]').text();
		 ...
		 function showAmountInWords(amount,id)
        {
           ...
           if($("#word_"+id).text() =='')
           {
                var dataString = 'amount=' + amount;
                    $.ajax({
                        type: "get",
                        url: "{{url('payment-request-view/show-amount-in-words')}}",
                        data: dataString,
                        success: function (result) {
                            if(result.status == 1){
    ...
    loadlink() // this will run after every 5 seconds
        ...
        <div class="panel panel-default">
			<div class="panel-body">
				<h4 class="page-title" style="color: black; font-size: 20px;">{{'Agent REQUEST VIEW' }}</h4>
            </div>
        </div>
    <!--Basic Columns-->
    <!--===================================================-->
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="table-responsive">
				<div class="col-lg-12">
				{{-- <marquee  behavior="slide"> <span class="marquee-span-tag"> Wallet Down :</span> <span style="color:red">{{ $down_api_lists}} </span></marquee> --}}
                <marquee onmouseover="this.stop();" onmouseout="this.start();"> <span class="marquee-span-tag"> Bank Down :</span> <span style="color:red">{{ $down_bank_list}} </span></marquee>
                ...
                <table id="mytable" class="table table-bordered">
                    ...
                    <tbody>
                    @foreach($loadcashs as $key => $value)
                        <tr>
                            ...
                            <td>{{ $value->user->name }} ( {{ $value->user->id }} )
                            	<input type="hidden" id="u_a_{{$value->id}}" value="{{ $value->user->name }} : {{ $value->user->role->role_title }} : {{$value->user->member->company }}">
                            </td>
                            <td>{{ @$value->user->member->company }}</td>
                            <td>{{ @$value->user->role->role_title }}</td>
                            <td>{{ $value->user->mobile }}</td>
                            <td><input type="hidden" id="pay_type_{{ $value->id }}" value="{{ @$value->pmethod->payment_type }}">{{ @$value->payment_mode }}</td>
							<td>{{ $value->loc_batch_code }}</td>
							<td>{{ $value->c_online_mode }}</td>
							<td>{{ $value->deposit_date }}</td>
                           <!-- <td><input type="hidden" id="bnk_nm_{{ $value->id }}" value="{{ @$value->netbank->bank_name }}">{{ @$value->netbank->bank_name }}</td>-->
						    <td>{{($value->request_to==1)?$value->bank_name:@$value->netbank->bank_name }}</td>
							...
							<!--{{ Form::open(array('url' => 'slip-update', 'method' => 'post', 'class' => 'form-light','files' => true)) }}
							{{ Form::file('d_slip', array('class' => 'form-control','id' => 'd_slip')) }}
							
							<div class="form-group">
							<input type="hidden" name="d_id" value="{{$value->id}}">
                            <input type="hidden" id="a_name_{{ $value->id }}" value="{{ @$value->user->name }}">
							</div>
							...
							@endif
							  ...
							  <td>{{ $value->bankref }}</td>
							@if(Auth::user()->role_id ==1)
								<td><a onclick="getLastTenApprovedAmount({{ $value->amount }})"   href="javascript:void(0)" >{{ $value->amount }}</a></td>
							@else
								<td>{{ $value->amount }}</td>
							@endif
							<!--<td><span id="word_{{$value->id}}"></span></td> -->
                            <td>{{ $value->status->status }}</td>
							<td>
							@if($value->status_id == 3)
                            <a onclick="updateRecord({{ $value->id }})" href="javascript:void(0)" class="table-action-btn"><i class="fa fa-pencil-square-o"></i></a>
							@endif
                            </td>
                            @if(Auth::user()->role_id == 1)
                            <td>
                               <a onclick="getRecords({{$value->user_id}})"  href="#" class="table-action-btn"  title="Show Last 5 Successful Txn"><i class="glyphicon glyphicon-th-list"></i></a>
                    ...
                    <form id="frmTasks" name="frmTasks" class="form-horizontal" novalidate="">
						...
						<div class="form-group row">
							<label class="col-sm-3 col-form-label text-right" for="paymentDate">Deposit/Approved Date:</label>
								...
								<div class="col-sm-5">
									<input type="text" class="form-control customDatepicker" id="approveDate" name="approveDate" value="{{date('d-m-Y H:i:s')}}" readonly >
								</div>
						</div>
                        <div class="form-group row">
							...
							@endif
                             ...
                             {!! Form::select('remark', ($remarks), null, ['class' => 'form-control','id'=>'remark_id']) !!}
                            </div>
							 @if(Auth::user()->role_id ==1)
