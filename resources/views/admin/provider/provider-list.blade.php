	...
	function addNewProvider()
	{
        ...
        /* function updateOperatorCode(id)
        {
            if(confirm('Are You sure to Update?'))
            {
				...
				$.ajax({
					type: "put",
					url: "{{url('update/operator-code')}}/"+id,
					data: dataString,
					dataType:"json",
	...
	function updateRecord(id) {
		...
		function updateProviderInfo()
        {
            if(confirm('Are you sure want to update?'))
            {
				...
				var url= url= "{{ url('provider/create') }}"
				if(requestType=="Update")
				{
					url= "{{ url('provider/update') }}/"+providerId;
				}
                $.ajax({
                    type: "POST",
                    url: url,
...
<div class="panel panel-default">
	<div class="panel-body">
		<div class="col-md-12">
			<div class="col-md-6">
				<h4 class="page-title" style="color: black; ">{{'Provider List' }}</h4>
			</div>
			<div class="col-md-6 pull-right">
		   <input type="search" id="search" value="" class="form-control" placeholder="Page Searching">
			<a href="{{ url('provider/un-categorised')}}" id="" class="btn btn-primary" >Un-Categorise Provider</a>
			<a href="{{ url('provider-list')}}" id="" class="btn btn-success" >Working Provider</a>
		...
		<table class="table table-bordered dataTable no-footer" id="example2" role="grid" aria-describedby="example2_info">
			...
			<tbody>
				...
				<tr>
					...
					<td>{{$provider->cyber}}</td>
					<td>{{$provider->redpay}}</td>
					<td>{{$provider->suvidhaa}}</td>
					<td>{{$provider->sp_key}}</td>
					<td>{{$provider->max_hold_txn}}</td>
					<td>{{$provider->max_hold_txn - $provider->hold_txn_couter}}</td>
					<td>{{$provider->min_pass_amt_txn}}</td>
					<td>{{$provider->service->service_name}}</td>
					<td><button type="button" onclick="updateRecord({{ $provider->id }})" class="btn btn-primary btn-sm" id="b_save_{{ $provider->id }}">Update Code</button></td>
					<td><span id="codeMessage_{{$provider->id}}" style="color:green"></span></td>	
				</tr>
				@endforeach
			</tbody>
		</table>  
                ...
                <form id="frmTasks" name="frmTasks" class="form-horizontal" novalidate="">
                    ...
                    <div class="modal-body">
						...
						<div class="form-group">
                            ...
                            <div class="col-sm-9">
                              {{ Form::select('serviceId', $servicelist, $provider->service_id, array('class' => 'form-control','id' => 'serviceId')) }}
                            </div>
                        </div>
                    </div>
