...
@section('content')
    ...
    // function txninprocess(id)
    // {
    //     var insert_id = id;
    //     var dataString = 'insert_id=' + insert_id;
    //     $.ajaxSetup({
    //             headers: {
    //                 'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
    //             }
    ...
    //     $.ajax({
    //         type: "get",
    //         url: "{{url('/txninprocess')}}",
    //         data: dataString,
    //         dataType: "json",
    //         beforeSend: function() {
    //                 $("#img_" + insert_id).show();
    //                 $("#btn_" + insert_id).remove();
                    
    //          },
    //         success: function (msg) 
    //         {
    //             $("#img_" + insert_id).remove();
    //             var host_name = 'https://' + location.hostname + '/mybal';
    //             if(msg.status==1)
    //             {
    //                 $("#process_" + id).addClass(msg.class).removeClass('btn btn-warning btn-sm');
    //                 $("#process_" + id).text(msg.message);
    //             } 
    //             else 
    //             {
    //                 swal("Failure", msg.message, "error");
    //                 $("#test_" + id).val('');
    //                 $("#btnnn_" + id).attr("disabled", false);
    //                 $("#btnnn_" + id).text("Send Now");
    //             }
    //         }

    //     });
    // }
    function hold_transaction(id)
    {
         var token = $("input[name=_token]").val();
          var insert_id = id;
            var dataString = 'insert_id=' + insert_id + '&_token=' + token;
        $.ajax({
                type: "GET",
                url: "{{url('ppay/holdtxn')}}",
                data: dataString,
                datatype: "json",
                beforeSend: function() {
                    $("#img_" + insert_id).show();
    ...
    function hold_transaction_rbl(id)
    {
         var token = $("input[name=_token]").val();
          var insert_id = id;
            var dataString = 'insert_id=' + insert_id + '&_token=' + token;
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="col-lg-6 col-md-6">
                <h4 class="page-title" style="color: white; font-size: 36px;">{{ $page_title or 'All In-process Transaction' }}</h4>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box">
                <h4 class="m-t-0 header-title"><b>{{ $page_title or 'All hold transaction Details' }}</b></h4>
                ...
                <table data-toggle="table"
                    ...
                    <tr>
                        ...
                        <th data-field="provider" data-align="center" >Product</th>
                        <th data-field="provider_name" data-align="center" data-sortable="true">Provider</th>
                        <th data-field="bank_name" data-sortable="true" data-formatter="dateFormatter">Bank Name
                        </th>
                        <th data-field="number" data-sortable="true" data-formatter="dateFormatter">Number</th>
                        <th data-field="txnid" data-sortable="true">Ref Id</th>

                        <th data-field="amount" data-align="center" data-sortable="true" data-sorter="priceSorter">Amount</th>
                        <th data-field="profit" data-align="center" data-sortable="true" data-sorter="priceSorter">S Charge</th>
                        <th data-field="total" data-align="center" data-sortable="true" data-sorter="priceSorter">Total</th>
                        <th>Status</th>
                        @if(Auth::user()->role_id == 1)
                            <th data-field="action" data-align="center" data-sortable="true">Action</th>
                            <th >Start Transfer </th>
                        @endif
                    </tr>
                    </thead>
                    ...
                    @if($value->status_id!=14)
                        <tr>
                            ...
                            <td><button id="process_{{ $value->id }}" class="btn btn-warning btn-sm" style="width:  100px;">{{ @$value->status->status }}<img src="images/load2.gif" height="40px" width="40px" id="img_{{$value->id}}" style="display:none" ></button></td>
                            @if(Auth::user()->role_id == 1)
                            ...
                            <td>
                                @if($value->api_id==3)
                                <button onclick="this.disabled=true;hold_transaction({{ $value->id }})" class=" btn btn-success" id="btn_{{$value->id}}">Start</button>
                                @elseif($value->api_id==4)
                                 <button onclick="this.disabled=true;hold_transaction_rbl({{ $value->id }})" class=" btn btn-success" id="btn_{{$value->id}}">Start</button>
                                 @endif
                            </td>
                            @endif
                        </tr>
