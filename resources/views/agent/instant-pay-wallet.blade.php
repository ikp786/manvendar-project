...
function varification() {
	...
	$.ajax({
		type: "GET",
		url: "{{url('instant-mobile-verification')}}",
		data: dataString,
		dataType: "json",
		beforeSend: function () {
			$("#messagedata").hide();
			...
			else if (msg.status == 'Transaction Successful') 
			{
				...
				else
				{
					...
					for (var key in obj2) 
					{
						...
						html += "<button class='btn btn-sm btn-primary tx-12' onclick='transfermodal(\""+rrid+"\",\""+raccount+"\")'>Pay</button> <button class='btn btn-sm tx-12 btn-danger' id='beneDeleteBtn_"+rrid+"' onclick='Deletebene("+rrid +")'> <i class='fa fa-trash'></i> </button>";
						}
						html += "</td>";
						html += "</tr>";
...
function addbene() {
	...
	var dataString = 'check_name=' + checkname + '&sessionid=' + sessionid + '&senderId=' + senderId + '&mobile_number=' + username + '&beneName=' + beneName + '&bbank=' + bbank + '&accountNumber=' + baccount + '&ifscCode=' + bifsc + '&number=' + bmobile + '&bankName=' + bankName + '&_token=' + token;
		$.ajax({
		type: "POST",
		url: "{{route('instant-add-beneficiary')}}",
		data: dataString,
		dataType: "json",
		beforeSend:function()
		{
...
function otpb() {
	...
	$.ajax({
		type: "GET",
		url: "{{route('instant-beneficiary-verification')}}",
		data: dataString,
		dataType: "json",
		success: function (msg) {
			$(".loader").hide();
...
function bene_resend_otp()
{
	...
	$.ajax({
		type: "get",
		url: "{{url('instant/resend-bene-otp')}}",
		data: dataString,
		dataType: "json",
		success: function (msg) {
			$(".loader").hide();
...
function Deletebene(beneid) 
{
	...
	$.ajax({
		type: "GET",
		url: "{{url('instant/delete_beneficiary')}}",
		data: dataString,
		dataType: "json",
		beforeSend:function()
		{
...
function Deletebene_confirm(){
	...
	$.ajax({
		type: "GET",
		url: "{{url('instant/confirm-bene-delete')}}",
		data: dataString,
		dataType: "json",
		beforeSend:function()
		{
...
function transferamount (){
	...
	$.ajax({
		type: "POST",
		url: "{{url('instant-api/transaction')}}",
		data: dataString,
		dataType: "json",
		beforeSend: function () {
...
$.ajax({
type: "get",
url: "{{url('cyber-api/beneconform_resend_otp')}}",
data: dataString,
dataType: "json",
success: function (msg) {
$(".loader").hide();
...
$.ajax({
type: "GET",
url: "{{url('cyber-api/bene_confirm')}}",
data: dataString,
dataType: "json",
success: function (msg) {
$(".loader").hide();
...
function remitterOTP()
{
	...
	if (remitterOTP != '') 
	{
		var dataString = 'mobileNumber=' + mobileNumber + '&remitterOTP=' + remitterOTP+ '&remitterVerifyId=' + remitterVerifyId;
			$.ajax({
			type: "GET",
			url: "{{url('instant/sender-verification')}}",
			data: dataString,
			dataType:"json",
			beforeSend:function()
			{
...
<div class="text-right">
	<a href="{{route('txn-history')}}?product=16&status_id=3" class="textheader">Pending </a>&nbsp;|&nbsp;
	<a href="{{route('txn-history')}}?product=16&status_id=4" class="textheader">Refunded</a>&nbsp;|&nbsp;
	<a href="{{route('txn-history')}}?product=16&status_id=20" class="textheader">Refund Pending</a>&nbsp;|&nbsp;
	<a href="{{route('txn-history')}}?product=16&status_id=1" class="textheader"> Transaction History</a>
</div>     
    ...
    <div class="col-md-4 mg-t-20">
            ...
            <div class="">
               <div class="form-group form-inline">
		            <div class="form-group">
					  ...
					  <button type="submit" class="btn btn-success btn-md" onclick="varification()" id="verificationBtn"><i class="fa fa-search"></i></button><img src="{{url('/loader/loader.gif')}}" id="loaderImg" class="loaderImg"> 
		                <a href="{{Request::url('')}}" class="btn btn-primary btn-md" ><i class="fa fa-refresh"></i></a>
					</div>					
			    </div>
				<div>
		...
		<div id="ben_frm" style="display: none;">
			...
			<div class="">
				...
				<img src="{{url('/loader/loader.gif')}}" id="beneLoaderImg" class="loaderImg" style="display:none"/>
				</div>
			...
			<div class="card-body ">
		  	...
		  	<div class="card-footer mg-t-auto">
				<button id ="registrationButton" class="btn btn-info bd-0 btn-oblong" onclick="registration();">Next Step</button>
				 <img src="{{url('/loader/loader.gif')}}" id="registrationLoader" class="loaderImg" style="margin-top: 16px;display:none"/>
		  	</div><!-- card-footer -->
		</div> 
			...
			<div class="card-body" id="">
		  	...
		  	<div class="card-footer mg-t-auto">
				<button id="remitterOTPButton" type="button" onclick="remitterOTP()" class="btn btn-primary">Confirm</button>
				   <img src="{{url('/loader/loader.gif')}}" id="remitterVerifyLoader" class="loaderImg" style="margin-top: 16px;display:none"/>
				   <button id="remitterResendOTPButton" type="button" onclick="remitterResendOTP()" class="btn btn-primary">Resend</button>
				   <img src="{{url('/loader/loader.gif')}}" id="remitterResendVerifyLoader" class="loaderImg" style="margin-top: 16px;display:none"/>
		  	</div><!-- card-footer -->
		</div>
            ...
            <div class="modal-dialog modal-md" role="document">
              <div class="modal-content tx-size-md" style="">
                ...
                <div class="modal-footer">
				  ...
				  <img src="{{url('/loader/loader.gif')}}" id="loaderTrnsImg" class="loaderImg" style="margin-top: 16px;display:none"/>
            ...
            <div class="modal-dialog modal-sm" role="document">
              <div class="modal-content tx-size-sm">
                ...
                <div class="modal-footer">
				  ...
				  <img src="{{url('/loader/loader.gif')}}" id="confirmloaderImg" class="loaderImg" style="margin-top: 16px;display:none"/>
                </div>
              </div>
            </div><!-- modal-dialog -->
          </div><!-- modal --> 
...
<div id="preTxnSlip" class="modal fade">
	...
	<div class="modal-footer">
	   <button onclick="checkIsBankDownOrNot()" type="button" class="btn btn-info" value="add" id="payAndConfirmBtn">Pay & Confirm</button>
	    ...
	    <img src="{{url('/loader/loader.gif')}}" id="aftertxnLoaderImg" class="loaderImg" style="display:none"/>
