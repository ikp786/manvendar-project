...
function addbene() {
	...
	var dataString = 'check_name=' + checkname + '&sessionid=' + sessionid + '&senderid=' + senderId + '&mobile_number=' + username + '&name=' + bname + '&bbank=' + bbank + '&bank_account=' + baccount + '&ifsc=' + bifsc + '&number=' + bmobile + '&bankcode=' + service_id + '&_token=' + token;
		$.ajax({
		type: "GET",
		url: "{{url('cyber-api/add_beneficiary')}}",
		data: dataString,
		dataType: "json",
		beforeSend:function(){
			$("addBeneBtn").hide()
...
$.ajax({
type: "GET",
url: "{{url('cyber-api/delete_beneficiary')}}",
data: dataString,
dataType: "json",
success: function (msg) {
$(".loader").hide();
...
$.ajax({
type: "GET",
url: "{{url('cyber-api/bene_confirm_delete')}}",
data: dataString,
dataType: "json",
success: function (msg) {
$(".loader").hide();
...
<div class="text-right">
	<a href="{{route('transaction-report')}}?product=4&searchOf=3" class="textheader">Pending </a>&nbsp;|&nbsp;
	<a href="{{route('transaction-report')}}?product=4&searchOf=4" class="textheader">Refunded</a>&nbsp;|&nbsp;
	<a href="{{route('transaction-report')}}?product=4&searchOf=20" class="textheader">Refund Pending</a>&nbsp;|&nbsp;
	<a href="{{route('transaction-report')}}?product=4&searchOf=1" class="textheader"> Transaction History</a>
</div>     
			...
			<a href="#" class="pull-right"  onClick="getTransactionOfMobile()" style="display:none" id="reportButton">Report</a>
               ...
               <div class="d-flex" style="padding: 2%;">
		            ...
		            <div class="mg-l-10 mg-t-25 pd-t-4">
		                <button type="submit" class="btn btn-success btn-md" onclick="callAutoFuncion()" id="verificationBtn"><i class="fa fa-search"></i></button><img src="{{url('/loader/loader.gif')}}" id="loaderImg" class="loaderImg"> 
          ...
          <div id="transfermodal" class="modal fade">
            <div class="modal-dialog modal-md" role="document">
              <div class="modal-content tx-size-md" style="">
               ...
               <!-- <div class="form-group">
                    <input type="hidden" id="txn_pin" class="form-control" value="{{Auth::user()->profile->txn_pin}}">
                </div>-->
				...
				<div class="modal-body">
					<div class="col-md-12 from-inline">
						...
						<div id="dvContents">
							<table class="table">
								<tr> 
									<td><img src="{{asset('newlog/images/Logo168.png')}}" class="img-rounded" alt="a2z suvidhaa" width="50" height="50"></td><td class="pull-right"><span>Outlet Name : {{Auth::user()->member->company}}</span><br><span>Retailer Mob : {{Auth::user()->mobile}}</span></td>
								</tr>
							</table>
							...
							<table class="table">
								...
								<tr><td> Txn Date :</td><td><span id="slipTxnDate">{{date("Y-m-d H:i:s")}}</span></td></tr>
