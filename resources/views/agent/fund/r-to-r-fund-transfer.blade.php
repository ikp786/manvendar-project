...
@section('content')
	...
	@include('agent.fund.fund-type')<br>
            ...
            <div class="col-md-6">
                <form method="get" action="{{ Request::url()}}" class="form-inline"
                    ...
                    <div class="form-group">
                        {{ Form::select('SEARCH_TYPE', ['ID'=>'Id','MOB_NO'=>'Mobile No'], null, ['class'=>'form-control', 'style'=>"height: 10%;"]) }}
                    </div>
                     <div class="form-group">
					...
					<tbody>
						@foreach($retilerLists as $user)
							<tr>
								...
								@endif
								<td><input id="amount_{{ $user->id }}" type="text" name="amount" style="width:60px; text-align:center; margin-right:5px; height:34px;" onfocusout="amountInWords({{ $user->id }})"><br><span id="amountInWords_{{$user->id}}" style="font-size: 11px;font-weight: bold;"></span>
								</td>
								...
								<button id="trbutton_{{ $user->id }}" onclick="transfernow({{ $user->id }})" class="btn btn-success">Transfer Now</button> </td>-->
								<td><center><span id="imgr_{{ $user->id }}" style="display:none;"><img src="images/load2.gif" height="40px" width="40px"></span></center>
								<button id="trbutton_{{ $user->id }}" onclick="openVerificationPin({{ $user->id }},'TXN')" class="btn btn-success">Transfer Now</button> </td>
								<!--<a onclick="openVerificationPin({{ $user->id }},'PROFILE_UPDATE')" href="javascript:void(0)" class="table-action-btn"><i class="md md-edit"></i>Update</a>-->
							</tr>
                        @endforeach
                        </tbody>
                    </table>
