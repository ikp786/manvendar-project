...
function my_varification() {
	...
	var dataString = 'mobile_number=' + username + '&_token=' + token;
	$.ajax({
		type: "GET",
		url: "{{url('my-wallet-mobile-verification')}}",
		data: dataString,
		dataType: "json",
		beforeSend: function () {
...
function my_get_bene()
	{
		   var mobile_number = $("input#number").val();
            if (mobile_number!='') 
			{
                ...
                $.ajax({
                    type: "get",
                    url: "{{url('/my-wallet-get-bene-list')}}",
                    data: dataString,
                    dataType: "json",
                    beforeSend: function () {
                        $("#verificationBtn").hide();
                    ...
                    success: function (msg) {
						   ...
						   if(msg.status == 22)
						   {
								...
								for (var key in obj2) 
								{
									...
									var mobile_no = obj2[key].customer_number;
									var bank_verified = obj2[key].bank_verified;
									var verfy ='';  
									if(bank_verified=='1'){
									    verfy = '<i class="fa fa-check-circle" style="color: #5cb85c;" title="Verified"></i>';
									}
									...
									html += "<tr>";
									html += "<td>"+rname+" "+verfy+"</td><td style='cursor: pointer; color:blue' onclick='getTranactionByField(\""+raccount+"\",\""+ACCOUNT_NUMBER+"\")'>"+raccount+"</td><td>"+bank_name+"</td><td>"+ifsc+"</td>";
									
									html += "<td>";
									
								//	html += "<div class='btn-group btn-sm  btn-primary' role='group'> <button class='btn btn-sm tx-12 btn-primary' onclick='my_bene_verifynow(\""+bank_name+"\",\""+ifsc+"\",\""+mobile_no+"\",\""+ACCOUNT_NUMBER+"\")'> Verify </button> </div>';
									
									html += "<button class='btn btn-sm btn-success' id='bload_"+raccount+"' onclick='my_bene_verifynow(\""+bank_name+"\",\""+ifsc+"\",\""+mobile_no+"\",\""+raccount+"\")'>Verify</button>";
									
									if (status == 0) {
									html += '<div class="btn-group btn-sm right" role="group"> <button class="btn btn-sm tx-12 btn-danger" onclick="pending_resend_top(\'' + rrid + '\')"> De Active </button> </div>';
									}else{                                                                                  
									html += "<button class='btn btn-sm btn-primary' onclick='transfermodal(\""+rrid+"\",\""+raccount+"\",\""+rname+"\")'>Pay</button><button class='btn btn-sm tx-12 btn-danger' onclick='Deletebene(\""+rrid+"\")'> <i class='fa fa-trash'></i> </button>";
									}
									html += "</td>";
									html += "</tr>";
							    ...
							    html += "<tr style='background: #d9edf763;'> <td class='text-center' colspan='5'>No Beneficiary Found.</td></tr>"
							}
...
function my_addbene(){
	...
	var beneficiaryStatus ='35';
	
	var account_digits_no=$("#account_digits_no").val();
	 
	/*if($.trim(baccount).length > account_digits_no){
	    alert('Account No is Invalid');
	    return false;
	}*/
	var dataString = 'mobile_number=' + username + '&beneName=' + bname  + '&accountNumber=' + baccount + '&ifscCode=' + bifsc + '&bankName=' + service_id + '&_token=' + token+'&caseType='+caseType+'&beneficiaryId='+beneficiaryId+'&status='+beneficiaryStatus;
		$.ajax({
		type: "POST",
		url: "{{url('my-wallet-bene-add')}}",
		data: dataString,
		dataType: "json",
		beforeSend:function(){
			$("addBeneBtn").hide()
...
function my_registration() {
	...
	if (fname != '' && lname != '') 
	{
		var dataString = 'fname=' + fname + '&lname=' + lname +  '&mobile_number=' + $("input#number").val() + '&_token=' + token;
			$.ajax({
			type: "GET",
			url: "{{url('my-wallet-remitter-register')}}",
			data: dataString,
			dataType: "json",
			beforeSend: function () {
                       // $("#registerBtn").hide();
			...
			success: function (msg) 
			{
			     ...
			     { 
			     	var errorString = '<div class="alert alert-danger"><ul><li>'+msg.message+'</li></ul>';
			     	$("#registerMissingr").show();
					$('#registerMissing').html(errorString); 
	...
	function my_otpVerification()
	{
		...
		var dataString = 'mobile=' + number +'&otp='+otp;
		 $.ajaxSetup({
			 ...
			 $.ajax({
                    type: "PUT",
                    url: "{{url('my-wallet-mobile-verification-with-otp')}}/"+number,
                    data: dataString,
                    dataType: "json",
                    beforeSend: function () {
                        $("#verifyOtpBtn").hide();
    ...
    function my_bene_verifynow(bankName,ifsc,mobile_no,bank_account) 
	{
		var token = $("input[name=_token]").val();
		...
		if (bank_account != '') 
		{
    		...
    		}else
    		{
    			...
    			var dataString = 'service_id=17' +'&bankcode=' + bankName + '&ifsc=' + ifsc + '&mobile_number=' + mobile_no + '&bank_account=' + bank_account + '&_token=' + token;
    			$(".loader").show();
    			$.ajax({
        			type: "GET",
        			...
        			beforeSend:function(){  
        				 ...
        				 $('#bload_'+bank_account).html('<i class="fa fa-spinner fa-spin"></i>'); 
        			},
        			success: function (msg) 
    			    { 
    			        if(msg.status_id == 201){
        				    alert(msg.message);
        				    $('#bload_'+bank_account).text('Verify');
        				}
        				else 
        				{
        				    if(msg.status=='Successful')
    					    {
    					        alert(msg.status+' '+msg.data.benename);
    					    }
    			            else{
        			                alert(msg.status);
        			            }
	...
	if(confirm("Are you sure want to delete beneficiary"))
	{
		...
		$.ajax({
			type: "put",
			url: "{{url('my-wallet-bene-delete-request')}}/"+beneId,
			data: dataString,
			dataType:"json",
			success: function (msg) {
				console.log(msg);
...
function Deletebene_confirm(){
	...
	$.ajax({
		type: "put",
		url: "{{url('my-wallet-confirm-bene-delete')}}",
		data: dataString,
		dataType: "json",
		beforeSend:function(){
...
function preTxnSlip()
{
	...
	var bankcode = $("#transferBank").val();
	var token = $("input[name=_token]").val();
	var mobile_no = $("#number").val();
    var trans_status = 1;
        // var trans_status = 1;
    	if(mobile_no != '') 
    	{
    	    var dataString = 'bankcode=' + bankcode + '&mobile_number=' + mobile_no + '&_token=' + token;
    			$.ajax({
    			type: "GET",
    			...
    			$("#imps_check").val(obj.is_imps_txn_allow);
    			if(obj.is_imps_txn_allow==0)
        			{ trans_status = 0;
        				alert("IMPS Transaction is not allowed, Please use DMT2 NEFT Mode");
        		        return false;
        			} 
	...
	if(trans_status=='1'){ 
        	...
        	dataString="amount="+ amount +"&txnChargeApiName=PAYTM";
        	$.ajax({
        		type: "GET",
        		url: "{{url('my-wallet-get-agent-charge-amt')}}",
        		data: dataString,
        		dataType: "json",
        		beforeSend:function()
        		{
        				...
        				if(msg.status == 0)
        					...
        					var content='';
        						$.each(msg.result, function (key, val) 
        						{
        							...
        							content +='<tr><td>'+val.txnAmount+'</td><td></td><td>'+val.total+'</td></tr>';
        						});
...
function paytm_transferamount(){

	var amount = $("#amount").val();
	if(amount <2 && amount >50000)   // amount limit
		{
...
var confirmAmount = $("#confirmAmount").val();
	...
	$.ajax({
		type: "POST",
		url: "{{url('my-wallet-paytm-transaction')}}",
		data: dataString,
		dataType: "json",
		beforeSend: function () {
		...
		success: function (msg) {
			...
			if (msg.status == 1||msg.status == 3||msg.status == 18 || msg.status == 34) 
			{
				refreshBalance();
				if(msg.status == 1){
					$("#slipTxnSataus").html("SUCCESS");
				    // $('#my_report_id').val(msg.txnId); oilab comment
				    my_payment_check(msg.txnId);
				}
				if(msg.status == 34){
					$("#slipTxnSataus").html("Accepted");
				    // $('#my_report_id').val(msg.txnId); oilab comment
				    my_payment_check(msg.txnId);
				}
				if(msg.status == 3)
...
function my_payment_check(report_id){ 
 
        setTimeout(function()
        {    
            // var report_id = $('#my_report_id').val();
            
            var token = $("input[name=_token]").val();
    	    
    		var dataString = 'id=' + report_id;
    		$.ajaxSetup({
    		headers: {
    			'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
    		}
...
function getIfsc() {
	...
	$('#account_digits_no').val(''); 
	$('#account_digit_div').text('');
	...
	if (number != '') 
	{
		var dataString = 'bankcode=' + bankcode + '&mobile_number=' + $("#number").val() + '&_token=' + token;
			...
			var account_digit = obj.account_digit;  
			
			$('#account_digits_no').val(account_digit);
            $('#account_digit_div').text('Account No. Max Length '+account_digit+' ');
...
function checkIsBankDownOrNot()
{
	...
	$.ajax({
		type: "GET",
		url: "{{url('my-wallet-check-is-bank-down')}}",
		data: dataString,
		dataType: "json",
		beforeSend:function()
		{
...
<div class="text-right">
	<a href="{{route('transaction-report')}}?product=25&searchOf=3" class="textheader">Pending</a>&nbsp;|&nbsp;
	<a href="{{route('transaction-report')}}?product=25&searchOf=4" class="textheader">Refunded</a>&nbsp;|&nbsp;
	<a href="{{route('transaction-report')}}?product=25&searchOf=20" class="textheader">Refund Pending</a>&nbsp;|&nbsp;
	<a href="{{route('transaction-report')}}?product=25&searchOf=1" class="textheader"> Transaction History</a>&nbsp;&nbsp;
</div>     
<div class="row row-sm" autocomplete="off">
          ...
          <div class="col-md-4 mg-t-20" autocomplete="off">
              <div class="card bd-0 shadow-base">
            	...
            	@include('partials.mobile-number-report')
            	  <img src="{{url('/loader/loader.gif')}}" id="mobileTxnLoader" class="loaderImg">
            ...
            <div class="mg-l-10 mg-t-25 pd-t-4">
                <button type="submit" class="btn btn-success" onclick="my_varification()" id="verificationBtn"><i class="fa fa-search"></i></button> 
			...
			<span style="font-weight: bold;" class="full-right"> <span>Remaining Limit : </span><span id="remainingLimits"></span></span>	
              ...
              <!-- <div class="col-md-4 mg-t-20" id="ben_frm"> -->
                ...
                <div class="" id="an" style="padding-bottom: 1%;">
                       ...
                       <input type="hidden" id="account_digits_no" value="" />
                       <p id="account_digit_div" style="text-align:right"></p>
					</div>	
						<span class="input-group-btn right">
						<button id="bnv" class="btn btn-primary mg-t-40-min" type="button"
						 onclick="my_verifynow();" style="float: right;">Verify</button>
						<img src="{{url('/loader/loader.gif')}}" id="accverifyLoaderImg" class="loaderImg" style="margin-top: 16px;display:none"/></span>
					...
					<div class="col-md-12">
						<div class="col-md-12">
						<button class="btn btn-info bd-0 btn-oblong" onclick="my_addbene();" id="addBeneBtn">Submit</button>
                  ...
                  <div class="card-footer mg-t-auto" id="addBeneDiv" style="display:none">
				   <button class="btn btn-info bd-0 btn-oblong" onclick="resendOtp();" id="resendOtpBtn">Resend OTP</button>
                    <button class="btn btn-info bd-0 btn-oblong" onclick="my_addbene();" id="addBeneBtn">Add Bene</button>
                 ...
                 <div class="card bd-0 shadow-base mg-t-20" id="registration" style="display: none;">
                  ...
                  <div class="card-footer mg-t-auto">
                    <button class="btn btn-info bd-0 btn-oblong" id ="registerBtn"onclick="my_registration();">Next Step</button>
				...
				<div class="card bd-0 shadow-base mg-t-20" id="otpVerificationDiv" style="display: none;">
                  ...
                  <div class="card-footer mg-t-auto">
                    <button id="verifyOtpBtn" class="btn btn-primary" type="button" onclick="my_otpVerification();">Verify </button>
					 <img src="{{url('/loader/loader.gif')}}" id="verifyLoaderImg" class="loaderImg" style="margin-top: 16px;display:none"/>
                   <button id="resendOtp" class="btn btn-success" type="button" onclick="my_varification();">Resend OTP </button>
                  </div><!-- card-footer -->
                </div>
            ...
            <div class="modal-dialog modal-md" role="document">
              <div class="modal-content tx-size-sm" >
                <div class="modal-header pd-x-20">
                    ...
                    <h4 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold bolder" style="color:#1e4a6f"><span class="benename"></span> - <span class="account_number"><</span></h4>
                  
                </div>
                {!!csrf_field()!!}
                <div class="modal-body pd-20">
                  <h4 class=" lh-3 mg-b-20 bank"></h4>
                  <input type="hidden" id="transferBank"> <input type="hidden" id="transferIfsc"> 
                  <input type="hidden" id="transferBenename">
    ...
    @include('common.transactionbyField')
...
<div id="preTxnSlip" class="modal fade">
	...
	<div id="printTxnSlip" class="modal fade">
			...
			<div class="form-group">
			  ...
			  <div class="modal-footer">
	   <button onclick="paytm_transferamount()" type="button" class="btn btn-info" value="add" id="payAndConfirmBtn">Pay & Confirm</button>
	    <img src="{{url('/loader/loader.gif')}}" id="txnLoaderImg" class="loaderImg" style="display:none"/>
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
      </div>
            </div><!-- modal-dialog -->
                  ...
                  <div class="modal-content tx-size-sm">
                        <div class="modal-body" style="padding: 10px; border-radius: 0px;">
                            <div class="col-md-12">
                                ...
                                <input type="hidden" id="my_report_id" />
                            </div>
                                                            ...
                                                            <td colspan="3" class="nospace1">
                                                                <table class="table table-bordered">
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td><span id="prt_tranoperator">{{"A2Z Plus Wallet"}}</span></td>
