...
<button class="btn-basic pull-right" title="Back" data-toggle="tooltip" data-placement="top"><a href="{{(URL::previous())}}" ><i class="fa fa-arrow-circle-left" aria-hidden="true" ></i></a></button>
 ...
 <div class="row col-md-12" >
    ...
    <form method="get" action="{{Request::url()}}" class="form-inline" role="form">
		...
		<form method="get" action="{{Request::url()}}" class="form-inline" role="form">
        <div class="form-group">
            <input name="fromdate" class="form-control customDatepicker" type="text" placeholder="From date" value="{{ (app('request')->input('fromdate')) ? app('request')->input('fromdate') : date('d-m-Y')}}" autocomplete="false">
        </div>
        <div class="form-group">
            <input name="todate" class="form-control customDatepicker" type="text" placeholder="To date" value="{{ (app('request')->input('todate')) ? app('request')->input('todate') : date('d-m-Y')}}" autocomplete="false">
        </div>
        <div class="form-group">
			<input type="hidden" name="product" value="{{@$product}}"/>
			<input type="hidden" name="status_id" value="{{@$status_id}}"/>
