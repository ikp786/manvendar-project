...
@include('agent.report.report-type')
	...
	<div class="col-md-12">
        <form method="get" action="{{Request::url()}}" class="form-inline" role="form">    
			...
			<input name="fromdate" style="width:140px" class="form-control customDatepicker" type="text" placeholder="From date" value="{{ (app('request')->input('fromdate')) ? app('request')->input('fromdate') : date('d-m-Y')}}">
		
			<input name="todate" style="width:140px" class="form-control customDatepicker" type="text" placeholder="To date" value="{{ (app('request')->input('todate')) ? app('request')->input('todate') : date('d-m-Y')}}">
			@if(in_array(Auth::user()->role_id,array(5)))
			{{Form::select('searchOf', ['34' => 'Accepted','1' => 'Success', '2' => 'Failed','3'=>"Pending",'4'=>"Refunded",'24'=>"Successfull",'21'=>"Refund Success",'18'=>"In-Process",'6'=>'DT'], (app('request')->input('searchOf')), ['class'=>'form-control','placeholder'=>"--Select Status--"])}}
			
			{{Form::select('product', ['1' => 'Recharge/Bill Payment', '2' => 'Verification','4' => 'DMT 1','16' => 'DMT 2','25'=>"A2Z Plus Wallet",'5'=>"A2Z wallet",'10'=>'AEPS'], (app('request')->input('product')), ['class'=>'form-control','placeholder'=>"--Select Product--"])}} 
			@else
			...
			@endif
			{{Form::select('mode', ['APP' => 'APP', 'WEB' => 'WEB'], (app('request')->input('mode')), ['class'=>'form-control','placeholder'=>"--Select Mode--"])}} 
