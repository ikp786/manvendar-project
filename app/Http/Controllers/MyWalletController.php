...
class MyWalletController extends Controller
{
	...
	public function index()
	{
		...
		if(in_array(Auth::user()->role_id,array(5,15)) && Auth::user()->member->paytm_my_wallet ==1)
		{
            ...
            $netbanks = Masterbank::where('status_id',1)->get(['bank_name','bank_code','account_digit']);
            //return $netbanks;
	...
	{	 
		...
		{
		    return Response::json(['status'=>21,'message'=>"Sender limit exhausted of A2Z Plus Wallet"]);
		}
		$statusId=3;
			...
			foreach($beneficiary_records as $content)
				{
					 $bene_arr[] =[ 'account_number'=>$content->account_number,
            			 ...
            			 'bank_verified' => $content->is_bank_verified,
            			 'name' => $content->name,
	...
	function beneAdd(Request $request)
	{
		...
		{ 	 
		    try{
		        ...
		        whereIn('api_id',['18','25'])->where('status_id','0')->count();
		        if($beneficiary_record_count<=0){
            		...
            		$beneDetails = Beneficiary::create([
		                ...
		                $beneficiary_records = Beneficiary::where(['account_number'=>$accountNumber,'ifsc'=>$ifsc,'mobile_number'=>$mobile_number])->
		                    whereIn('api_id',['18','25'])->where('status_id','0')->first(); 
		                if(!empty($beneficiary_records))
		                {          
                            $beneRecord = $beneficiary_records->id;
                            Beneficiary::where(['id'=>$beneRecord->id])->update(['status_id'=>1]); 
		                }
            ...
            if($beneficiary_records->otp!=$request->otp){
               return $mydata = Response::json(array(
                ...
                try{
        		    ...
        		    Beneficiary::where(['benficiary_id'=>$request->beneId])->update(['status_id'=>0]); 
