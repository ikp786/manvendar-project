...

{
    ...
    use CustomTraits, CustomAuthTraits,UserTraits;
...

    {
        ...
        $userQuery = User::selectRaw('id,prefix,name,email,mobile,balance_id,member_id,status_id,parent_id');

        if ($request->searchType == "NAME")

            $userQuery->where('name', 'like', trim($request->searchInput) . '%');

        elseif ($request->searchType == "MOB")

            $userQuery->where('mobile', 'like', trim($request->searchInput) . '%');

        elseif ($request->searchType == "ID")

            $userQuery->where('id', 'like', trim($request->searchInput) . '%');

        /*  if ($userDetails->role_id == 1) {

              if ($request->roleType == "ALL")

                  $userQuery->whereNotIn('role_id', [1, 3]);

              elseif ($request->roleType == "DISTRIBUTORS")

                  $userQuery->where('role_id', 4);

              elseif ($request->roleType == "RETAILERS")

                  $userQuery->where('role_id', 5);

          } else*/

        $userQuery->where('role_id', 5)->where('parent_id', $userDetails->id);

        $userDetails = $userQuery->simplePaginate(40);
        ...
        $members = $userDetails->map(function ($user) {
            ...
            return [
                ...
                'balance' => number_format($user->balance->user_balance, 2),

                'parentDetails' => $user->parent->name . '( ' . $user->parent->prefix . ' ' . $user->parent->id . ')',
        ...
        return response()->json(['status' => 1, 'members' => $members, 'count' => count($members), 'page' => "page=" . $pageNo,]);
    ...
    public function makeUserActiveDeactivate(Request $request)

    {

        $rules = ['userId' => 'required', 'token' => 'required', 'agentUserId' => 'required'];

        $validator = Validator::make($rules, $rules);

        if ($validator->fails())

            return response()->json(['status' => '10', 'message' => 'missing field', 'error' => $validator->getMessageBag()]);
        ...
        $agentUserDetails = User::find($agentUserId);



        if ($agentUserDetails->parent_id == $userId) {

            $agentUserDetails->status_id = 0;

            if ($agentUserDetails->save())
                ...
                return response()->json([
                    ...
                    'current_status' => $agentUserDetails->status_id

                ]);

            else return response()->json(['status' => '2', 'message' => 'failed to update user status! try again in sometime']);

        } else return response()->json(['status' => '2', 'message' => 'User not found!']);
...

    {
        ...
        if ($validator->fails()) return response()->json(
            ...
            ['status' => '10', 'message' => 'missing params', 'error' => $validator->getMessageBag()]);
        ...
        $states = State::where('id', '!=', 0)->pluck('name', 'id');

        return response()->json(['status' => '1', 'message' => 'success', 'states' => $states]);
...

    {
        ...
        if (Auth::user()->role_id == 1) {
            ...
            $schemes = Scheme::select('id', 'scheme_name')->where('company_id', '=', $company_id)->pluck('scheme_name', 'id')->toArray();
...

    {
        ...
        if (in_array(Auth::user()->role_id, array(1, 12))) {
            ...
            $bank_up_down = Masterbank::where('id', '!=', 1)->orderBy('bank_status', 'asc')->get();
...

    {
        ...
        Masterbank::where('id', $request->id)
            ...
            ->update(array('bank_status' => $updateStatus));
...

    {
        ...
        if ($request->id != '') {
            ...
            if ($update) {
                ...
                return "Somthing Wrong to Update!";
...

    {
        ...
        $parent_id = User::where('role_id', 5)
...

    {
        ...
        try {
            ...
            $message = "Dear Admin, " . $request->input('name') . " is  registered as " . $usern->role->role_title . " By " .$userDetails->name . ",Thanks!";
            ...
            try {
                ...
                CustomTraits::sendSms(config('constants.ADMIN_MOBILE_NO'), $message, $userDetails->company_id);

                $message = "Welcome to a2zsuvidhaa.com, We are glab to became a part of A2ZSUVIDHAA Member. Thanks A2ZSUVIDHAA.";

                $message = urlencode($message);

                CustomTraits::sendSms($user->mobile, $message, $userDetails->company_id);
            ...
            return response()->json(['status' => '1', 'message' => "Member added Successfully "]);

        } catch (Exception $e) {

            DB::rollback();

            return response()->json(['status' => '2', 'message' => 'Oops! Something went wrong '
...

    {
        ...
        if (in_array(Auth::user()->role_id, array(1))) {
            ...
            try {
                ...
                $member->blocked_amount = $request->blocked_amount;
...

    {
        ...
        $userQuery = User::selectRaw('id,name,mobile,role_id,parent_id,status_id,created_at,prefix');

        if ($request->searchType == "NAME")

            $userQuery->where('name', 'like', trim($request->contents) . '%');

        elseif ($request->searchType == "MOB")

            $userQuery->where('mobile', 'like', trim($request->contents) . '%');

        elseif ($request->searchType == "ID")

            $userQuery->where('id', 'like', trim($request->contents) . '%');
