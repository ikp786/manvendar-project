...
class MyWalletController extends Controller
{
	...
	{	 
		$rules = array(
			...
			'amount' => 'required|min:2|numeric|regex:/^[0-9]+$/', 
			'senderName' => 'required|regex:/^[A-Za-z ]+$/',			// amount limit
		...
		if($GetRemitterData->rem_bal<$request->amount)
		{
		    return Response::json(['status'=>666,'message'=>"Amount can not be greater than remaining limit ".$GetRemitterData->rem_bal]);
		}
        $appResp = ApiResponseToApp::create([
            'response_from_app' => json_encode($request->all()),
            'request_type' => 'A2Z Plus Wallet',
		...
		$isBankDetails = Masterbank::select('bank_status','bank_name')->where('bank_code',$bankCode)->first();
		...
		$duplicatae = $this->checkDuplicateTransaction($bank_account,$amount,$userId,$mobile_number,25);
		...
		try
		{ 
			$finalAmount = $amount+$agentCharge-$agentComm;
		...
		$apiResp = Apiresponse::create(['api_id'=>25,'user_id'=>$userId,'api_type'=>"TRANSACTION",'report_id'=>$record->id,'request_message'=>json_encode($content)]);
        ...
        // echo "<br/>paytmapi resp ="; print_r($apiResponse); die;
			...
			try
			{
				...
				if($result->status=="SUCCESS" || $result->status=="ACCEPTED")//($result->status==1)
				{
					...
					$this->creditCommission($d_id,$m_id,$a_id,$userId,$mobile_number,$beneficiary_id,2,$insert_id,$amount,$dist_charge_data,$md_charge_data,$admin_charge_data,$bank_account,$record,25);
            			...
            			if(!empty($GetRemitterData) && $GetRemitterData->rem_bal!='0'){  
                            ...
                            try{  
                              //  RemitterRegistration::where(['mobile'=>$mobile_number])->update(['rem_bal'=>$less_amt,'updated_at'=>date('Y-m-d H:i:s')]); 
                        		...
                        		// }
								$response =['txnId'=>$my_txn_id,'refNo'=>$my_ref_id,'status'=>1,'message'=>"Transaction $result->status"];
								$appResp->response_to_app = json_encode($response);
								$appResp->save();
								 return response()->json($response);
				...
				{  
				    ...
				    if($result->statusCode=='DE_704' || $result->statusCode=='DE_705')
					{
					...
					if($result->statusCode=='DE_010' || $result->statusCode=='DE_039' || $result->statusCode=='DE_040' || $result->statusCode=='DE_602' || $result->statusCode=='DE_634' || $result->statusCode=='DE_641'){
    					...
    					try
    					{
    						...
    						$this->updateRemitterAmount($insert_id);
    						DB::commit();
    					}
    					catch(Exception $e)
    					{
				    ...
				    || $result->statusCode=="DE_656" )
				    {
				        	return response()->json(['status'=>2,'message'=>$result->statusMessage]);
				    }
				    else if($result->statusCode=="DE_704" || $result->statusCode=="DE_705"){
				            return response()->json(['status'=>2,'message'=>"Internal Server Error"]);
				    }
				    
					return response()->json(['status'=>2,'message'=>"Transaction $r_message"]);
				}
			//	elseif($result->status=="FAILURE") //(in_array($result->status,array(10,101,100,11,12,34,21,29,18,19,22,29,23,24,26,21,301,503,31,30)))
			/*	{  
