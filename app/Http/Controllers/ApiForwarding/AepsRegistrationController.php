...
class AepsRegistrationController extends Controller
{
    ...
    function aepsMemberRegistration(Request $request)
	{
		...
		$storeApiResp = AepsUserRegistrationResponse::create(['request_from_api'=>json_encode($request->all()),'user_id'=>$userdetail->id,'mobile'=>$request->mobile,'pan_number'=>$request->panNumber,'aadhaar_number'=>$request->aadhaarNumber,'aeps_registration_id'=>'0']);
		...
		try
		{
		    ...
		    $storeApiResp->aeps_registration_id=$apiRegistration->id;
		    	$storeApiResp->save();
		}
		catch(Exception $e)
		{
		...
		$merchantLoginPin=$request->merchantLoginPin;
