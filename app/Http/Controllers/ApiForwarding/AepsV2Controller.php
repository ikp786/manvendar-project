...
class AepsV2Controller extends Controller
{
	...
	public function sendRequest(Request $request)
	{
		...
		try
		{
			$apiRegDetails = ApiAepsAgentRegistration::where(['user_id'=>$userId,'agent_id'=>$request->agentId])->firstOrFail();
			if($apiRegDetails->status==3)
			{
				$respToApi = [
					'status'=>85,	
					'message' => "Agent onboarding process is pending. Contact with admin",
				];
				$storeApiResp->response_to_api = json_encode($respToApi);
				$storeApiResp->save();
				return Response::json($respToApi);
		...
		catch(Exception $e)
		{
			$respToApi = [
			'status'=>84,	
			'message' => "Agent is not registred,Please Onbard First",
			];
			$storeApiResp->response_to_api = json_encode($respToApi);
			$storeApiResp->save();
			return Response::json($respToApi);
		...
		else
		{
			...
			$post = array('captureResponse'=>
				...
				'merchantUserName'=>$apiRegDetails->merchant_login_id,
				'merchantPin'=>md5($apiRegDetails->merchant_login_pin),
				...
				else
				{
					if($transactionType=="CW")
					{
						...
						try
						{
							...
							'status'=>2,
							'availableBalance'=>$result->data->balanceAmount,
							'transactionAmount'=>$result->data->transactionAmount,'bankRRN'=>@$result->data->bankRRN,
							//'transactionType'=>@$result->data->transactionType,
							'transactionType'=>$transactionType,
							'txnId'=>$aepsReport->id,
